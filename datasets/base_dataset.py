...
def create_mask(landmarks, shape, extend_to_forehead=True):
    # Convert landmarks to the required format for convexHull function
    landmarks = landmarks.astype(np.int32)[...,:2]
    # print(landmarks)
    if extend_to_forehead:
        # move eyebrows up by 20 pixels
        landmarks[19:27,1] -= 25
        # move left eyebrows a bit to the left
        landmarks[19:22,0] -= 10
...
class BaseDataset(torch.utils.data.Dataset):
    def __init__(self, data_list, config, test=False, sample_full_video_for_testing=False):
        ...
        self.test = test
        self.K = config.K
        self.sample_full_video_for_testing = sample_full_video_for_testing
    ...
    def sample_frames(self, imagePathsOrVideo, landmarksPathsOrNumpyArrayFan, landmarksPathsOrNumpyArrayMediapipe):
        ...
        for frame_idx in frame_indices:
            ...
            # print(self.name, kpt_mediapipe.shape)

            # if flag_landmarks_fan is False:
            tform = self.crop_face(frame,kpt_mediapipe,scale)
            
            kpt_mediapipe = kpt_mediapipe[...,:2]
            
            # else:
                # tform = self.crop_face(frame,kpt_fan,scale)
            ...
            # find convex hull
            hull_mask = create_mask(cropped_kpt_mediapipe, (self.image_size, self.image_size), extend_to_forehead=False)

            cropped_kpt_mediapipe = cropped_kpt_mediapipe[mediapipe_indices,:2]

            # hull_mask_rev = 1 - hull_mask
            # print(cropped_kpt_mediapipe)
        ...
        # if kpt_array.shape[-2] == 68:
        #     flag_landmarks = True
        # else:
        #     flag_landmarks = False
        #     kpt_array = kpt_array[:,:68,:] # Just do this so that it can collate with the others
