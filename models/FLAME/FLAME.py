...
class FLAME(nn.Module):
    ...
    def __init__(self, flame_model_path='assets/FLAME2020/generic_model.pkl', 
                 flame_lmk_embedding_path='assets/FLAME2020/landmark_embedding.npy', n_shape=100, n_exp=50, subject_template_path=None):
        ...
        if subject_template_path is None:
            print('Using generic FLAME model')
            self.register_buffer('v_template', to_tensor(to_np(flame_model.v_template), dtype=self.dtype))
        else:
            from psbody.mesh import Mesh
            mean_mesh = Mesh(filename=subject_template_path)
            self.register_buffer('v_template', to_tensor(to_np(mean_mesh.v), dtype=self.dtype))
            print('Loaded subject template from: ', subject_template_path)
    ...
    def forward(self, shape_params=None, expression_params=None, jaw_params=None, pose_params=None, eye_pose_params=None, eyelid_params=None, neck_pose_params=None):
        """
            Input:
                shape_params: N X number of shape parameters
                expression_params: N X number of expression parameters
        ...
        #    jaw_params = self.jaw_pose.expand(batch_size, -1) #torch.zeros([batch_size, 3], dtype=self.dtype, device=shape_params.device)
...
class FLAMETex(nn.Module):
    ...
    def __init__(self):
        ...
        pc_key = 'PC'
        n_pc = 199
        tex_path = 'assets//FLAME_albedo_from_BFM.npz'
